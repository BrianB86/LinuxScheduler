...
static inline int fair_task_time(struct task_struct * p)
{
	unsigned long num_proc_running = 1;
	unsigned long num_user_running = 1;
	unsigned int result;
	...
	list_for_each(curr, &runqueue_head) {
		tmp = list_entry(curr, struct task_struct, run_list);
		if(tmp->policy == SCHED_OTHER && tmp->pid != pid)
		{
			num_proc_running++;
			if(tmp->uid == uid)
			{
	...
	return ((p->nice * num_proc_running) / 2) / num_user_running; //change 2 to a function call to count number of users
}
